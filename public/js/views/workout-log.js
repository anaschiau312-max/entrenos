...
const WorkoutLogView = {
    ...
    renderForm() {
        ...
        // Form fields per type FIRST (manual entry is primary)
        ...
        if (session.type === 'running' || session.type === 'cycling') {
            html += this.renderOcrSection(false); // expanded by default for running
        } else if (session.type === 'strength_upper' || session.type === 'strength_lower') {
            html += this.renderOcrSection(true); // collapsed by default for strength
        }
    ...
    renderReference(session) {
        const icon = session.type === 'running' ? '🏃' : session.type === 'cycling' ? '🚴' : session.type === 'strength_upper' ? '💪' : session.type === 'strength_lower' ? '🦵' : '💪';
        const badgeClass = session.type === 'running' ? 'badge-running' : session.type === 'cycling' ? 'badge-cycling' : (session.type === 'strength_upper' || session.type === 'strength_lower') ? 'badge-strength' : 'badge-strength';
        const typeLabel = session.type === 'running' ? 'Carrera' : session.type === 'cycling' ? 'Ciclismo' : session.type === 'strength_upper' ? 'Fuerza tren superior' : session.type === 'strength_lower' ? 'Fuerza tren inferior' : 'Fuerza';
        ...
        <div class="card wl-ocr-card" id="wl-ocr-section" data-expandable>
            <div class="expandable-header wl-ocr-header ${collapsedClass}" data-toggle="ocr-expand">
                <div class="wl-ocr-title-row">
                    ...
                    <div>
                        <div class="card-title">Captura del reloj <span class="wl-optional-tag">(opcional)</span></div>
                        <div class="text-xs text-muted">Sube una captura para extraer datos automáticamente</div>
                    </div>
                </div>
                <span class="expand-icon">
            ...
            <div class="expandable-body wl-ocr-body ${bodyClass}">
                ...
                <div id="wl-ocr-upload-area" class="wl-ocr-upload-area">
                    <div class="wl-ocr-upload-content">
                        ...
                        <span class="wl-ocr-upload-text">Pulsa para subir captura</span>
    ...
    mount() {
        ...
        document.querySelectorAll('[data-toggle="ocr-expand"]').forEach(header => {
            header.addEventListener('click', () => {
                header.classList.toggle('expanded');
                const body = header.nextElementSibling;
                body.classList.toggle('open');
    ...
    validate(session) {
        ...
        if (session.type === 'strength_upper' || session.type === 'strength_lower') {
            const h = parseInt(document.getElementById('wl-dur-h').value) || 0;
            const m = parseInt(document.getElementById('wl-dur-m').value) || 0;
            const sec = parseInt(document.getElementById('wl-dur-s').value) || 0;
            if (h + m + sec === 0) return 'Introduce la duración del entrenamiento.';
    ...
    collectFormData(session) {
        ...
        if (session.type === 'strength_upper' || session.type === 'strength_lower') {
            ...
            base.actual = {
                completed: true,
                duration,
                calories: parseInt(document.getElementById('wl-calories').value) || null,
                heart_rate_avg: parseInt(document.getElementById('wl-hr').value) || null,
                heart_rate_max: parseInt(document.getElementById('wl-hr-max').value) || null,
                zone_percentage: parseInt(document.getElementById('wl-zone-pct').value) || null
            };
