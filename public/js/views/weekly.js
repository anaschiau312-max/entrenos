...
const WeeklyView = {
    ...
    renderWeek() {
        ...
        for (const dateStr of sortedDates) {
            ...
            // Expandable details - show description for all sessions
            let detailsHtml = '';
            for (const session of sessions) {
                if (session.description) {
                    const sIcon = session.type === 'running' ? '🏃' :
                                  session.type === 'strength_lower' ? '🦵' :
    ...
    mount() {
        ...
        document.querySelectorAll('.week-edit-day-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                sessionStorage.setItem('editPlanWeek', weekNum);
                sessionStorage.setItem('editPlanDate', dateStr);
                if (window.EditPlanView) {
                    window.EditPlanView.state.currentWeekNum = weekNum;
                    window.EditPlanView.state.editingDate = dateStr;
                }
