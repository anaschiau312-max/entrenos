...
const weeks = {
        ...
        days: {
            ...
            "2026-02-20": {
                sessions: [{
                    ...
                    description: "Fuerza completa (activación + fuerza principal + core). Curl femoral deslizante en vez de peso muerto rumano si no hay máquina",
                    duration: 40,
                    cooldown: "post-fuerza-inferior"
                }],
                notes: "Trabajo hasta 14:30. Secuencia: Calentamiento → carrera → vuelta a la calma → fuerza inferior → vuelta a la calma"
            },
            "2026-02-21": {
                sessions: [{
                    type: "running",
                ...
                notes: "Libre. Paseo ok este día"
            },
            "2026-02-22": {
                sessions: [{
                    type: "running",
                    title: "Tirada larga",
                    description: "Cinta: empezar a 7.5-7.8 km/h los primeros 5-10 min. Subir a 8.0-8.4 km/h zona cómoda. Si a partir del min 35-40 necesitas abrir la boca, bajar a 7.5-7.8 km/h. FC se registra pero el regulador principal es la respiración nasal. Objetivo FC media: ≤158. Evaluar rodilla durante y 24h después",
                    duration: 50,
    ...
    "week_07": {
        ...
        context: "Mismos horarios que semana 6. Consolidación antes de descarga",
        successIndicators: [
            "Tirada larga de 55 min completada sin dolor de rodilla",
            "FC media estable o menor que semana 6 a ritmo similar",
            "Longitud de paso ≥0.81m (sin retroceso al patrón protector)",
        ...
        days: {
            ...
            "2026-02-27": {
                sessions: [{
                    ...
                    type: "strength_lower",
                    title: "Fuerza tren inferior",
                    description: "Fuerza completa. Curl femoral deslizante o en máquina. Practicar patrón bisagra de cadera sin peso si hay tiempo",
                    duration: 40,
                    cooldown: "post-fuerza-inferior"
                }],
                notes: "Trabajo hasta 14:30"
            },
            "2026-02-28": {
                sessions: [{
                    type: "running",
                ...
                notes: "Libre. Paseo ok pero no excesivo (máx 2-3h)"
            },
            "2026-03-01": {
                sessions: [{
                    type: "running",
                    title: "Tirada larga",
                    description: "Cinta: empezar a 7.5-7.8 km/h los primeros 5-10 min. Subir a 8.0-8.4 km/h zona cómoda. Si necesitas abrir la boca, bajar un punto. FC se registra, regulador principal es respiración nasal. Progresión +5 min vs semana 6. Evaluar: FC media (≤158), rodilla durante y 24h post, momento de abrir boca",
                    duration: 55,
                    warmup: "pre-carrera-larga",
                    cooldown: "post-carrera-larga"
                }],
...
const speedReference = {
    ...
    note: "Todas las sesiones de carrera al 1% inclinación",
    speeds: [
...
const alertSignals = [
    ...
    "FC que sube >10 lpm en últimos 15 min vs primeros 15 min (deriva cardíaca)",
    "Sensación de rodilla que no desaparece en primeros 10 min",
    "Fatiga que no remite con 48h de descanso",
    "Asimetría contacto suelo >52/48"
];

async function uploadAll() {
    try {
        console.log('Subiendo datos con formato correcto...');

        // Upload weeks with correct key format
        for (const [weekKey, weekData] of Object.entries(weeks)) {
            console.log(`Subiendo ${weekKey}...`);
            await db.ref(`plan/weeks/${weekKey}`).set(weekData);
        }
